    ...
    async def load_all_mongodb(self, mdb):
        ...
        try:
            ...
            if ldclient.get().variation("data.monsters.gridfs", context, False) or config.TESTING:
                ...
                self.raw_monsters = lookup.get("monsters", self.raw_monsters)
        except:
            self.raw_monsters = lookup.get("monsters", self.raw_monsters)
        self.raw_backgrounds = lookup.get("backgrounds", self.raw_backgrounds)
        self.raw_adventuring_gear = lookup.get("adventuring-gear", self.raw_adventuring_gear)
        self.raw_armor = lookup.get("armor", self.raw_armor)
        self.raw_magic_items = lookup.get("magic-items", self.raw_magic_items)
        self.raw_weapons = lookup.get("weapons", self.raw_weapons)
        self.raw_races = lookup.get("races", self.raw_races)
        self.raw_subraces = lookup.get("subraces", self.raw_subraces)
        self.raw_spells = lookup.get("spells", self.raw_spells)
        self.raw_books = lookup.get("books", self.raw_books)
        self.raw_actions = lookup.get("actions", self.raw_actions)
